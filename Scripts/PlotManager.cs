...
public class NewBehaviourScript : MonoBehaviour
{
            ...
            switch (fm.selectedTool)
            {
                ...
                case 3:
                    if (fm.money >= 100 && !isBought) 
                    {
                        ...
                        isBought = true;
                        plot.sprite = drySprite;
                        fm.IncrementAvailablePlotCount();
                    }
                    break;
                default:
                    break;
    ...
    void Harvest()
    {
        ...
        ShowCoin();
        fm.IncrementHarvestedPlantCount();
    }
    ...
    void ShowCoin()
    {
        ...
        // Add SpriteRenderer component to the coin object
        SpriteRenderer coinRenderer = coinObject.AddComponent<SpriteRenderer>();
        coinRenderer.sprite = coinSprite;

        // Set the sorting order of the coin renderer to be in front of the plot
        coinRenderer.sortingOrder = plot.sortingOrder + 1;

        // Add Rigidbody2D and Collider2D components for physics interactions
        Rigidbody2D coinRb = coinObject.AddComponent<Rigidbody2D>();
        coinRb.gravityScale = 1.0f;

        CircleCollider2D coinCollider = coinObject.AddComponent<CircleCollider2D>();
        coinCollider.isTrigger = true;

        // Apply force to make the coin move upwards
        coinRb.AddForce(Vector2.up * 5.0f, ForceMode2D.Impulse);

        // Destroy the coin object after a short delay
        Destroy(coinObject, 1.0f);
    }
